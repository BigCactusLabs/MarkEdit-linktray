...
`,v="markedit-linktray-root",W="style[data-linktray-style]";function q(e,t){let r=0,n="",i=!1;return{get selectedIndex(){return r},get query(){return n},render(){return U(y(e,n,i),r,n)},handleKey(a){const o=y(e,n,i).interactiveEntries;if(a==="ArrowDown"){if(o.length===0)return;r=Math.min(r+1,o.length-1);return}if(a==="ArrowUp"){r=Math.max(r-1,0);return}if(a==="Enter"){const l=o[r];if(l?.type==="missing-summary"){i=!i,r=Math.min(r,y(e,n,i).interactiveEntries.length-1);return}h(l?.item,t);return}a==="Escape"&&t.onClose()},click(a){r=a;const s=y(e,n,i).interactiveEntries[r];if(s?.type==="missing-summary"){i=!i;return}h(s?.item,t)},hover(a){r=a},setQuery(a){n=a,r=0}}}function B(e){return async t=>{const r=e.document??globalThis.document;if(!r?.body||!r?.head)return;V(r),Y(r);const n=r.createElement("div");n.id=v,r.body.append(n);let i=!1,a=null;const s=()=>{i||(i=!0,r.removeEventListener("keydown",u,!0),r.removeEventListener("keyup",d,!0),r.removeEventListener("keypress",d,!0),n.remove())},o=q(t,{onOpen:async m=>{await e.openFile(m),s()},onClose:async()=>{s()}}),l=()=>{i||(n.innerHTML=o.render(),a=n.querySelector(".linktray-search"),H(n,o,l),a?.focus(),a?.setSelectionRange(a.value.length,a.value.length))},u=m=>{i||!a||r.activeElement!==a||x(m.key)&&(m.preventDefault(),m.stopPropagation(),o.handleKey(m.key),!i&&m.key!=="Escape"&&l())},d=m=>{i||!a||r.activeElement!==a||x(m.key)&&(m.preventDefault(),m.stopPropagation())};return r.addEventListener("keydown",u,!0),r.addEventListener("keyup",d,!0),r.addEventListener("keypress",d,!0),l(),e.onShow?.(o),o}}function U(e,t,r){const n=e.interactiveEntries.map((l,u)=>{if(l.type==="missing-summary")return[`<button class="linktray-summary${u===t?" linktray-item--selected":""}" data-index="${u}" type="button">`,`<span class="linktray-summary__label">Missing (${l.count})</span>`,'<span class="linktray-summary__meta">Show hidden links</span>',"</button>"].join("");const d=l.item,m=["linktray-item",d.status==="missing"?"linktray-item--missing":"linktray-item--existing",u===t?"linktray-item--selected":""].filter(Boolean).join(" "),S=d.status==="missing"?' aria-disabled="true"':"";return[`<li class="${m}" data-index="${u}"${S}>`,`<span class="linktray-item__symbol linktray-item__symbol--${d.status}" aria-hidden="true"></span>`,'<div class="linktray-item__body">',`<span class="linktray-item__filename">${p(d.filename)}</span>`,`<span class="linktray-item__path">${p(d.displayPath)}</span>`,"</div>","</li>"].join("")}),i=n.slice(0,e.availableItems.length).join(""),a=e.showMissingSummary?1:0,s=a?n[e.availableItems.length]??"":"",o=n.slice(e.availableItems.length+a).join("");return['<section class="linktray-overlay" role="dialog" aria-label="Linked Markdown files">','<div class="linktray-panel">','<header class="linktray-toolbar">','<div class="linktray-toolbar__eyebrow">Index</div>','<div class="linktray-toolbar__headline">','<h2 class="linktray-toolbar__title">Linked Markdown</h2>',`<div class="linktray-toolbar__count">${p(G(e.availableItems.length,e.missingItems.length))}</div>`,"</div>","</header>",'<div class="linktray-searchRow">','<label class="linktray-searchRow__label" for="linktray-search">Filter</label>',`<input id="linktray-search" class="linktray-search" type="text" placeholder="Type a file, path, or status" value="${p(r)}" />`,"</div>",'<div class="linktray-body">',e.availableItems.length>0?'<span class="linktray-sectionLabel">Available</span>':"",e.availableItems.length>0?`<ul class="linktray-list">${i}</ul>`:"",s,e.missingItems.length>0&&!e.showMissingSummary?'<span class="linktray-sectionLabel">Missing</span>':"",o?`<ul class="linktray-list">${o}</ul>`:!i&&!s?'<p class="linktray-empty">No linked notes match this filter.</p>':"","</div>","</div>","</section>"].join("")}async function h(e,t){e?.openPath&&await t.onOpen(e.openPath)}function p(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function w(e,t){const r=t.trim().toLowerCase();return r?e.filter(n=>`${n.filename}
${n.displayPath}
${n.status}`.toLowerCase().includes(r)):e}function y(e,t,r){const n=w(e.filter(o=>o.status==="existing"),t),i=w(e.filter(o=>o.status==="missing"),t),a=t.trim().length>0&&i.length>0,s=i.length>0&&!a&&!r;return{availableItems:n,missingItems:i,showMissingSummary:s,interactiveEntries:[...n.map(o=>({type:"item",item:o})),...s?[{type:"missing-summary",count:i.length}]:[],...s?[]:i.map(o=>({type:"item",item:o}))]}}function H(e,t,r){const n=e.querySelector(".linktray-search"),i=e.querySelectorAll("[data-index]");n?.addEventListener("input",a=>{t.setQuery(a.currentTarget.value),r()}),i.forEach(a=>{a.addEventListener("click",()=>{const s=Number(a.dataset.index);Number.isNaN(s)||(t.click(s),r())}),a.addEventListener("mouseenter",()=>{const s=Number(a.dataset.index);Number.isNaN(s)||(t.hover(s),r())})})}function V(e){if(e.head.querySelector(W))return;const t=e.createElement("style");t.dataset.linktrayStyle="true",t.textContent=K.trim()||Q,e.head.append(t)}function Y(e){e.getElementById(v)?.remove()}function x(e){return e==="ArrowDown"||e==="ArrowUp"||e==="Enter"||e==="Escape"}function G(e,t){return`${e} active · ${t} missing`}function J(e,t=X(e)){const r=async()=>{const n=await e.getFileInfo();if(!n?.filePath){await e.showAlert({title:"MarkEdit-linktray unavailable",message:"Open a saved Markdown file before running MarkEdit-linktray."});return}const i=await e.getFileContent(),a=$(i??"");if(a.length===0){await e.showAlert({title:"No linked Markdown files",message:"The current document does not contain any Markdown links to show."});return}const s=await Z(e,n.filePath),o=await Promise.all(a.map(async l=>{const u=j({currentFilePath:n.filePath,rawTarget:l.rawTarget,kind:l.kind}),d=await e.getFileInfo(u);return{index:l.index,resolvedPath:u,displayPath:C({currentFilePath:n.filePath,resolvedTargetPath:u,repoRootPath:s}),exists:d!==void 0}}));await t(L(o))};return e.addMainMenuItem({title:"Open Linked Markdown",key:"L",modifiers:["Command","Shift"],action:r}),r}function X(e){return e.openFile?B({openFile:t=>e.openFile?.(t)??Promise.resolve(!1)}):async()=>{}}async function Z(e,t){let r=g(c(t));for(;;){if(await e.getFileInfo(z(r,".git")))return r;const i=g(r);if(i===r)return null;r=i}}J(E.MarkEdit);
